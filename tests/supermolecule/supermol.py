...
def compute_interaction_energy(monomer_a, monomer_b, basis='cc-pVDZ', xc='b3lyp'):
    ...
    output_AB = compute_dft(mol_AB, [CALCS.ENERGY])
    E_AB = output_AB["energy"]

    output_A = compute_dft(mol_A, [CALCS.ENERGY])
    E_A = output_A["energy"]

    output_B = compute_dft(mol_B, [CALCS.ENERGY])
    E_B = output_B["energy"]    

    output_A_ghost = compute_dft(mol_A_ghost, [CALCS.ENERGY])
    E_A_ghost = output_A_ghost["energy"]    

    output_B_ghost = compute_dft(mol_B_ghost, [CALCS.ENERGY])
...
monomer_b_ghost = "\n".join([*monomer_a.split("\n"), *["ghost":_  for _ in monomer_b.split("\n")]])
